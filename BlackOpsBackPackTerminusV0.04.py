...
root.geometry("200x700")  # Compacte breedte en hoogte
root.attributes("-alpha", 0.75)  # Transparantie
...
# Function to toggle visibility (F2 key functionality)
def toggle_visibility(event=None):
    if root.state() == 'iconic':  # Check if the window is minimized
...
root.bind("<F2>", toggle_visibility)
